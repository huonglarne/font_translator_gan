...
class DFS_Encoder(nn.Module):
    def __init__(
        self, norm_layer=nn.BatchNorm2d, padding_type="reflect", mode="content"
    ):
        ...
        self.down_conv_1 = nn.Conv2d(
            64, 192, kernel_size=3, stride=2, padding=1, bias=False
        )
        self.down_norm_1 = norm_layer(192)
        self.down_nonlinear_1 = nn.ReLU(True)

        self.down_conv_2 = nn.Conv2d(
            192, 576, kernel_size=3, stride=2, padding=1, bias=False
        )
...
class DFS_Decoder(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d):
        ...
        self.up_conv_2 = nn.ConvTranspose2d(
            1728, 192, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False
        )
        self.up_norm_2 = norm_layer(192)
        self.up_nonlinear_2 = nn.ReLU(True)

        self.up_conv_1 = nn.ConvTranspose2d(
            384, 64, kernel_size=3, stride=2, padding=1, output_padding=1, bias=False
        )
...
class DFS_Mixer(nn.Module):
    ...
    def forward(self, inp):
        ...
        sim_mat = F.softmax(
            sim_mat * 2, dim=1
        )  # the constant a is set to 2 to adjust their relative distance
    ...
    def get_similarity(self, CT_feature, CR_features):
        ...
        return torch.sum(
            CT_feature * CR_features, dim=[3, 4], keepdim=True
        ) / torch.sqrt(torch.sum(CR_features**2, dim=[3, 4], keepdim=True))
