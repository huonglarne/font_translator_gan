...
class DFSModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            parser.set_defaults(
                batch_size=64, pool_size=0, gan_mode="lsgan", netD="basic_64"
            )
            parser.add_argument(
                "--lambda_L1", type=float, default=0.5, help="weight for L1 loss"
            )
            parser.add_argument(
                "--lambda_L2", type=float, default=0.5, help="weight for L2 loss"
            )
        return parser
    ...
    def set_input(self, data):
        ...
        if self.isTrain:
            self.visual_names = ["gt_images", "generated_images"] + [
                "IR_images_{}".format(i) for i in range(self.IR_images.shape[1])
            ]
        else:
    ...
    def forward(self):
        ...
        self.generated_images = self.netG(
            (self.IR_images, self.CR_images, self.CT_images)
        )
    ...
    def backward_D(self):
        ...
        self.loss_D = self.compute_gan_loss_D(
            self.gt_images, self.generated_images, self.netD
        )
    ...
    def backward_G(self):
        ...
        self.loss_G_L2 = (
            self.criterionL2(self.generated_images, self.gt_images) * self.opt.lambda_L2
        )
