...
class DFSDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        self.dataroot = os.path.join(
            opt.dataroot, opt.phase, opt.direction.split("2")[1]
    ...
    def get_IR_paths(self, parts):
        IR_font_path = os.path.join(
            parts[0], parts[1], parts[2], parts[3], self.table[parts[4]], parts[5]
        )
    ...
    def get_CR_paths(self, parts, characters):
        return [
            os.path.join(parts[0], parts[1], parts[2], parts[3], "source", character)
